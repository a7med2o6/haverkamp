...
/**
 * مستند الفاتورة — ورق الشركة الرسمي، أو ورقة A4 بيضاء، أو إيصال حراري بعرض 80مم.
 *
      ...
      */}
      <div className="flex flex-col items-center gap-1 border-b border-[var(--line)] pb-4 text-center">
          <Image
            ...
            height={40}
            className={cn(
              "w-auto object-contain invoice-logo",
              receipt ? "h-7" : "h-9",
            )}
          />
          <p
            className={cn(
              "font-bold text-[var(--text-0)]",
              receipt ? "text-[13px]" : "text-base",
            )}
          >
            هافركامب الكويت
          </p>
      ...
      {doc.void && (
        <div className="mt-4 rounded-[var(--radius-sm)] border border-danger/30 bg-danger/10 px-3.5 py-2.5">
          <p className="font-semibold text-danger">
            {doc.status === "REFUNDED"
              ? "فاتورة مرتجعة — رُدّ ما حُصِّل منها"
              : "فاتورة ملغاة"}
          </p>
          {/* السبب ملاحظة موظفين — قد تُكتب بلا تحفّظ، فلا تُرسل للعميل */}
          {staff && doc.void.reason && (
            <p className="mt-0.5">{doc.void.reason}</p>
          )}
      ...
      >
        <Meta label="رقم الفاتورة">
          <span
            className="tnum text-base font-bold text-[var(--text-0)]"
            dir="ltr"
          >
            {doc.number}
          </span>
        ...
        <Badge tone={ORDER_STATUS[doc.status].tone}>
          {ORDER_STATUS[doc.status].label}
        </Badge>
      </section>

      {/* ── العميل والسيارة ── */}
      <section
        className={cn("mt-4 grid gap-4", !receipt && car && "sm:grid-cols-2")}
      >
        <Meta label="العميل">
          {doc.customer ? (
            <>
              ...
              {staff && doc.customer.phone && (
                <span
                  className="tnum block text-[12px] text-[var(--text-2)]"
                  dir="ltr"
                >
          ...
          <Meta label="السيارة">
            ...
            <span className="block text-[12px] text-[var(--text-2)]">
              {[
                ...
                doc.job?.odometer != null &&
                  `${doc.job.odometer.toLocaleString("en-US")} كم`,
              ]
                .filter(Boolean)
                .join(" · ")}
            </span>
          ...
          {doc.job && (
            <>
              ...
              {jobHref ? (
                <Link
                  href={jobHref}
                  className="tnum text-accent hover:underline print:no-underline"
                  dir="ltr"
                >
                  {doc.job.number}
                </Link>
      ...
      {/* ── الإجماليات ── */}
      <section
        className={cn("mt-4 space-y-1.5", !receipt && "ms-auto max-w-xs")}
      >
            ...
            {doc.payments.map((p) => (
              ...
              >
                <span className="flex flex-wrap items-center gap-2">
                  ...
                  {p.reference && (
                    <span
                      className="tnum text-[11px] text-[var(--text-2)]"
                      dir="ltr"
                    >
                      {p.reference}
                    </span>
                ...
                <span
                  className={cn(
                    "tnum",
                    p.amount < 0 ? "text-danger" : "text-[var(--text-1)]",
                  )}
                >
                  {formatKWD(p.amount)}
                </span>
          ...
          <ul className="space-y-1">
            {doc.job.warranties.map((w) => (
              ...
              >
                ...
                <span className="tnum text-[11px] text-[var(--text-2)]">
                  <span dir="ltr">{w.certificateNo}</span> · حتى{" "}
...
function Row({
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      <span
        className={
          strong ? "font-bold text-[var(--text-0)]" : "text-[var(--text-2)]"
        }
      >
        {label}
      </span>
